...
def _fallback_evidence(
    *,
    ...
    return FigureEvidence(
        ...
        compared_items=(artifact.legend_items if artifact and artifact.legend_items else _infer_compared_items(figure)),
        metrics_or_axes=_sanitize_list(metrics, max_items=6, max_length=80),
        visible_text=_sanitize_list(artifact.visible_text if artifact else [], max_items=24, max_length=160),
        legend_items=_sanitize_list(artifact.legend_items if artifact else [], max_items=12, max_length=120),
        panels=artifact.panels if artifact else [],
